...
\icmltitlerunning{Online Coresets for symmetric Tensor Factorization and $\ell_{p}$ Regression}

\begin{document}

\twocolumn[
\icmltitle{Online Coresets for symmetric Tensor Factorization and $\ell_{p}$ Regression}
...
\begin{abstract}
Tensors are high dimensional arrays. Factorizing tensors has recently become an important optimization module in a number of machine learning pipelines, especially in latent variable models. We show how to do this efficiently in an online setting, using low space and time. Given a set of $n$ vectors $\*a_{i}$, each in $\~R^{d}$, represented by the matrix $\*A$, and a rank-$k$ subspace $\*Q \subset \~R^d$, we present an online algorithm, that creates a subset $\*C$ of 
%size $O(n^{1-2/p}dk(1+2\log(\norm{\*A})))$
sublinear input size 
while using a working space $O(d^2)$. This subset $\*C$ can be used to get an well approximated decomposed factors. We show that a sampling based $\ell_{p}$ subspace embedding coreset can also be used in learning latent variables through tensor factorization.
...
% 
% The sampled vectors also work as (online) relative error coresets for an $\ell_p$ regression problem of the form $\min_\*x \|\*A\*x - \*b\|_p$. 
% The sampling complexity reduces significantly under the assumption that the vectors come as a randomly ordered stream. 
...
% As a specific application, we are able to apply tensor factorization to do topic modeling for a document stream in one pass using the above mentioned space. In case the documents come from a generative model, a sample size of $O(\frac{dk}{\epsilon^{2}}\log(n)\log(1/\delta))$ is sufficient. 
